...
class IdaOperations:
    ...
    def get_functions(offset=0, limit=100, filter_str=None):
        ...
        for sea in idautils.Functions():
            ...
            funcs.append(
                {"name": name, "address": hex(sea), "size": func.size() if func else 0}
            )
        if limit <= 0:
            return funcs[offset:]
    ...
    def scan_aob(pattern):
        ...
        # Support IDA 8.x backwards compatibility by falling back to idc.find_binary
        try:
            # First try the newer compiled binpat matching (IDA 8.3+)
            ...
            # Fallback for IDA <= 8.2 using find_binary
            ea = idc.find_binary(min_ea, idc.SEARCH_DOWN | idc.SEARCH_NEXT, pattern)
    ...
    def wait_bpt(timeout):
        ...
        res = ida_dbg.wait_for_next_event(
            ida_dbg.WFNE_CONT | ida_dbg.WFNE_SUSP, timeout
        )
        if res == 1:
            # Hit breakpoint
            ctx = {}
    ...
    def get_strings(offset=0, limit=100, filter_str=None):
        ...
        for idx, s in enumerate(sc):
            ...
            if filter_str and filter_str.lower() not in val.lower():
                continue
            strings.append({"address": hex(s.ea), "value": val})
        if limit <= 0:
    ...
    def get_exports(offset=0, limit=100):
        ...
        for i in range(ida_entry.get_entry_qty()):
            ...
            if name:
                exports.append({"address": hex(ea), "name": name})
        if limit <= 0:
    ...
    def get_xrefs_to_field(struct_name, field_name):
        ...
        for ref in idautils.XrefsTo(tid):
            xrefs.append(
                {"address": hex(ref.frm), "type": "code" if ref.iscode else "data"}
            )
        return xrefs
    ...
    def declare_c_type(c_declaration):
        ...
        if sys.platform == "win32":
            ...
            if errors > 0:
                raise Exception("Parse decls failed with code " + str(errors))
            return True
        else:
            flags = ida_typeinf.PT_SIL | ida_typeinf.PT_EMPTY | ida_typeinf.PT_TYP
    ...
    def get_stack_frame_variables(address):
        ...
        struc = ida_struct.get_struc(frame_id)
        if not struc:
            return []
            
        members = []
        # IDA 8.x and older struct member enumeration
        for offset in range(0, ida_struct.get_struc_size(struc)):
            ...
            if member:
                ...
                if name and offset == member.soff: # only add at start of member
                    size = ida_struct.get_member_size(member)
                    # Try to get type info if possible
                    mtype = ""
                    try:
                        import ida_typeinf
                        tif = ida_typeinf.tinfo_t()
        ...
        for ordinal in range(1, type_count):
            try:
                ...
                if tif.get_numbered_type(idati, ordinal):
                    type_name = tif.get_type_name() or (
                        "<Anonymous #" + str(ordinal) + ">"
                    )
                    type_str = type_name
                    locals_list.append(
                        {"ordinal": ordinal, "name": type_name, "declaration": type_str}
                    )
            except:
                continue
        return locals_list
                        ...
                        members = [
                            {
                                "name": x.name,
                                "offset": hex(
                                    (x.offset // 8) if hasattr(x, "offset") else 0
                                ),
                                "size": hex((x.size // 8) if hasattr(x, "size") else 0),
                                "type": str((x.type) if hasattr(x, "type") else ""),
                            }
                            for x in udt
                        ]
                except AttributeError:
    ...
    def analyze_struct_detailed(name):
        ...
        if not tif.get_named_type(None, name):
            raise Exception("Structure " + name + " not found")
        if not tif.is_udt():
            raise Exception("Not a UDT")
        ...
        try:
            ...
            for i, m in enumerate(udt_data):
                ...
                members.append(
                    {
                        "index": i,
                        "offset": hex(m.begin() // 8) if hasattr(m, "begin") else 0,
                        "size": size,
                        "type": m.type._print(),
                        "name": m.name,
                        "is_nested_udt": m.type.is_udt(),
                    }
                )
        except AttributeError:
            # Fallback for IDA 8.x
            import ida_struct
            struct_id = ida_struct.get_struc_id(name)
            if struct_id == idaapi.BADADDR:
                raise Exception("Failed to get details via struct fallback")
            struc = ida_struct.get_struc(struct_id)
            for offset in range(0, ida_struct.get_struc_size(struc)):
                member = ida_struct.get_member(struc, offset)
                if member:
                    mname = ida_struct.get_member_name(member.id)
                    if mname and offset == member.soff:
                        members.append({
                            "index": len(members),
                            "offset": hex(member.soff),
                            "size": ida_struct.get_member_size(member),
...
class MCPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # ── Data Extraction ────────────────────────────────────────
                ...
                result = IdaOperations._execute_sync_write(
                    IdaOperations.patch_bytes, args.get("address"), args.get("hex_bytes")
                )
            else:
            ...
            if result is not None:
                ...
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"error": "No result generated"}).encode("utf-8"))
    ...
    def set_bpt(address):
        import ida_dbg
        addr = address if isinstance(address, int) else int(address, 16)
        return ida_dbg.add_bpt(addr, 0, ida_dbg.BPT_DEFAULT)

    @staticmethod
    def wait_bpt(timeout=15):
        import ida_dbg
        # IDA's wait_for_next_event is often sufficient
        return ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, timeout)
