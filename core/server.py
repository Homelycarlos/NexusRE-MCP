...
def scaffold_kernel_interface(game_name: str, with_cr3: bool = False) -> Any:
...
#define IOCTL_BASE  CTL_CODE(FILE_DEVICE_UNKNOWN, 0x802, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
static ULONG64 GetProcessCr3(PEPROCESS Process) {{
    return *(ULONG64*)((ULONG64)Process + 0x28); // KPROCESS.DirectoryTableBase
}}

static NTSTATUS ReadVirtualMemory(PEPROCESS Process, PVOID Source, PVOID Target, SIZE_T Size) {{
    SIZE_T bytes = 0;
    return MmCopyVirtualMemory(Process, Source, PsGetCurrentProcess(), Target, Size, KernelMode, &bytes);
}}

static NTSTATUS WriteVirtualMemory(PEPROCESS Process, PVOID Source, PVOID Target, SIZE_T Size) {{
    SIZE_T bytes = 0;
    return MmCopyVirtualMemory(PsGetCurrentProcess(), Source, Process, Target, Size, KernelMode, &bytes);
}}
...
NTSTATUS DeviceControl(PDEVICE_OBJECT DeviceObject, PIRP Irp) {{
    ...
    if (controlCode == IOCTL_READ) {{
        PEPROCESS process = NULL;
        if (NT_SUCCESS(PsLookupProcessByProcessId((HANDLE)req->ProcessId, &process))) {{
            status = ReadVirtualMemory(process, (PVOID)req->Address, (PVOID)req->Buffer, (SIZE_T)req->Size);
            ObDereferenceObject(process);
        }}
    }} else if (controlCode == IOCTL_WRITE) {{
        PEPROCESS process = NULL;
        if (NT_SUCCESS(PsLookupProcessByProcessId((HANDLE)req->ProcessId, &process))) {{
            status = WriteVirtualMemory(process, (PVOID)req->Buffer, (PVOID)req->Address, (SIZE_T)req->Size);
            ObDereferenceObject(process);
        }}
    }} else if (controlCode == IOCTL_BASE) {{
        PEPROCESS process = NULL;
        if (NT_SUCCESS(PsLookupProcessByProcessId((HANDLE)req->ProcessId, &process))) {{
            req->DirectoryBase = GetProcessCr3(process);
            ObDereferenceObject(process);
        }}
    }}

    Irp->IoStatus.Status = status;
    Irp->IoStatus.Information = sizeof(KERNEL_RW_REQUEST);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {{
    ...
    UNICODE_STRING devName = RTL_CONSTANT_STRING(L"\\\\Device\\\\{game_name}Drv");
    UNICODE_STRING symLink = RTL_CONSTANT_STRING(L"\\\\DosDevices\\\\{game_name}Drv");
...
NTSTATUS DeviceControl(PDEVICE_OBJECT DeviceObject, PIRP Irp) {{
    PIO_STACK_LOCATION stack = IoGetCurrentIrpStackLocation(Irp);
    ULONG controlCode = stack->Parameters.DeviceIoControl.IoControlCode;
    // MmCopyVirtualMemory-based read
    return STATUS_SUCCESS;
}}
"""
...
public:
    DriverClient() {{
        hDriver = CreateFileA("\\\\\\\\.\\\\{game_name}Drv", GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
    }}
    ~DriverClient() {{ if (hDriver != INVALID_HANDLE_VALUE) CloseHandle(hDriver); }}
    ...
    T Read(uint64_t pid, uint64_t addr) {{
        ...
        req.ProcessId = pid;
        req.Address = addr;
        req.Buffer = (uint64_t)&value;
        req.Size = sizeof(T);
        DWORD bytes = 0;
        DeviceIoControl(hDriver, IOCTL_READ, &req, sizeof(req), &req, sizeof(req), &bytes, nullptr);
        return value;
    }}
    ...
    bool Write(uint64_t pid, uint64_t addr, const T& value) {{
        ...
        req.Size = sizeof(T);
        DWORD bytes = 0;
        return DeviceIoControl(hDriver, IOCTL_WRITE, &req, sizeof(req), &req, sizeof(req), &bytes, nullptr);
    }}

    uint64_t GetCR3(uint64_t pid) {{
        KernelRWRequest req{{}};
        req.ProcessId = pid;
        DWORD bytes = 0;
        DeviceIoControl(hDriver, IOCTL_BASE, &req, sizeof(req), &req, sizeof(req), &bytes, nullptr);
        return req.DirectoryBase;
    }}
}};
"""
        return {
            ...
            "with_cr3": with_cr3,
            "message": f"Kernel driver + client scaffolded for '{game_name}' with {'CR3-based' if with_cr3 else 'standard'} memory access."
        }
    except Exception as e:
         return handle_error(e)
...
async def session_management_tools(
    ...
    else: return {"error": f"Unknown session_management action: {action}"}
...
async def function_navigation_tools(
    ...
    else: return {"error": f"Unknown function_navigation action: {action}"}
...
async def binary_extraction_tools(
    ...
    else: return {"error": f"Unknown binary_extraction action: {action}"}
...
async def decompilation_tools(
    ...
    else: return {"error": f"Unknown decompilation action: {action}"}
...
async def memory_debugging_tools(
    ...
    else: return {"error": f"Unknown memory_debugging action: {action}"}
...
async def modification_tools(
    ...
    else: return {"error": f"Unknown modification action: {action}"}
...
async def structural_tools(
    ...
    else: return {"error": f"Unknown structural action: {action}"}
...
async def signature_scanning_tools(
    ...
    else: return {"error": f"Unknown signature_scanning action: {action}"}
...
async def game_dumping_tools(
    ...
    else: return {"error": f"Unknown game_dumping action: {action}"}
...
async def ai_intelligence_tools(
    ...
    else: return {"error": f"Unknown ai_intelligence action: {action}"}
...
async def binary_analysis_sandbox(
    ...
    else: return {"error": f"Unknown binary_analysis_sandbox action: {action}"}
...
async def export_sync_tools(
    ...
    else: return {"error": f"Unknown export_sync action: {action}"}
...
async def frida_scripting_tools(
    ...
    else: return {"error": f"Unknown frida_scripting action: {action}"}
...
def knowledge_base_tools(
    ...
    else: return {"error": f"Unknown knowledge_base action: {action}"}
...
async def history_cache_tools(
    ...
    else: return {"error": f"Unknown history_cache action: {action}"}
...
def background_event_poller():
    ...
    while True:
        try:
            ...
            logger.debug(f"Event poller error: {e}")
        time.sleep(5.0)  # 5s interval — was 0.1s (10Hz), which burned CPU
...
def inject_shellcode(pid: int, shellcode_hex: str, technique: str = "createremotethread") -> Any:
    ...
    try:
        ...
        if not kernel32.WriteProcessMemory(h_process, arg_address, shellcode, len(shellcode), ctypes.byref(written)):
            kernel32.VirtualFreeEx(h_process, arg_address, 0, MEM_RELEASE)
            return {"error": f"WriteProcessMemory failed. Error: {kernel32.GetLastError()}"}
        ...
        return {"error": f"Unknown injection technique: {technique}"}
    except Exception as e:
        return handle_error(e)
    finally:
        if h_process:
...
def generate_detour_hook(session_id: str, address: str, convention: str = "__fastcall", proto: str = "void* rcx") -> Any:
    """Generates a ready-to-compile C++ inline detour hook using a 14-byte JMP relay with prologue backup and trampoline."""
    template = f"""
#include <windows.h>
#include <cstdint>
#include <cstring>
...
constexpr uintptr_t TARGET_ADDRESS = {address};

// Original function typedef
typedef void ({convention}* OriginalFn_t)({proto});
OriginalFn_t OriginalFunction = nullptr;

// Trampoline buffer — stores the backed-up prologue + JMP back
static uint8_t TrampolineBuffer[64] = {{}};

// Our detour function
void {convention} HookedFunction({proto}) {{
    // Custom logic here — runs before original
    // e.g.: modify rcx, log call, skip original, etc.

    // Call original via trampoline
    if (OriginalFunction) {{
        OriginalFunction({proto.split()[-1] if ' ' in proto else 'rcx'});
    }}
}}

// 14-byte absolute JMP: FF 25 00000000 [8-byte address]
static void WriteAbsoluteJmp(uint8_t* dst, uintptr_t target) {{
    dst[0] = 0xFF;
    dst[1] = 0x25;
    *(uint32_t*)(dst + 2) = 0x00000000;
    *(uint64_t*)(dst + 6) = target;
}}

bool InstallDetour() {{
    uint8_t* target = (uint8_t*)TARGET_ADDRESS;
    constexpr size_t HOOK_SIZE = 14; // Size of our JMP relay
    constexpr size_t STOLEN_BYTES = 16; // Align to instruction boundary (>= HOOK_SIZE)

    // 1. Backup the original prologue into the trampoline
    memcpy(TrampolineBuffer, target, STOLEN_BYTES);

    // 2. Append a JMP back to (target + STOLEN_BYTES) after the stolen bytes
    WriteAbsoluteJmp(TrampolineBuffer + STOLEN_BYTES, (uintptr_t)(target + STOLEN_BYTES));

    // 3. Make trampoline executable
    DWORD oldTrampolineProt;
    VirtualProtect(TrampolineBuffer, sizeof(TrampolineBuffer), PAGE_EXECUTE_READWRITE, &oldTrampolineProt);

    // 4. Store the trampoline as the "original" function pointer
    OriginalFunction = (OriginalFn_t)TrampolineBuffer;

    // 5. Overwrite the target prologue with a JMP to our hook
    DWORD oldProt;
    VirtualProtect(target, HOOK_SIZE, PAGE_EXECUTE_READWRITE, &oldProt);
    WriteAbsoluteJmp(target, (uintptr_t)&HookedFunction);
    VirtualProtect(target, HOOK_SIZE, oldProt, &oldProt);
...
bool RemoveDetour() {{
    uint8_t* target = (uint8_t*)TARGET_ADDRESS;
    constexpr size_t STOLEN_BYTES = 16;

    DWORD oldProt;
    VirtualProtect(target, STOLEN_BYTES, PAGE_EXECUTE_READWRITE, &oldProt);
    memcpy(target, TrampolineBuffer, STOLEN_BYTES); // Restore original bytes
...
"""
    return {
        ...
        "prototype": proto,
        "type": "Detour",
        "note": "Stolen bytes size (STOLEN_BYTES) must be >= 14 and aligned to instruction boundaries. Disassemble at target to verify."
    }
...
async def exploitation_tools(
    ...
    else: return {"error": f"Unknown exploitation action: {action}"}
