...
class ExdnspyAdapter(BaseAdapter):
    """
    Adapter mapping NexusRE-MCP tool calls to the dnSpyEx/exdnspy background
    HTTP server running on the local machine (Port 10106).
    ...
    async def _call(self, action: str, args: dict = None) -> dict:
        ...
        # Caching for read-only mass data endpoints
