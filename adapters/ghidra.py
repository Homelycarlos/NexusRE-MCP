...
class GhidraAdapter(BaseAdapter):
    ...
    async def list_functions(self, offset: int = 0, limit: int = 100, filter_str: Optional[str] = None) -> List[FunctionSchema]:
        res = await self._call("ghidra_list_functions", {"offset": offset, "limit": limit, "filter": filter_str})
        return [
            FunctionSchema(
                name=f.get('name', ''),
    ...
    async def get_current_address(self) -> Optional[str]:
        res = await self._call("ghidra_get_current_address")
        return res.get("address") if res else None

    async def get_current_function(self) -> Optional[str]:
        res = await self._call("ghidra_get_current_function")
        return res.get("address") if res else None
    ...
    async def get_strings(self, offset: int = 0, limit: int = 100, filter_str: Optional[str] = None) -> List[StringSchema]:
        res = await self._call("ghidra_get_strings", {"offset": offset, "limit": limit, "filter": filter_str})
    ...
    async def get_globals(self, offset: int = 0, limit: int = 100, filter_str: Optional[str] = None) -> List[GlobalVarSchema]:
        res = await self._call("ghidra_get_globals", {"offset": offset, "limit": limit, "filter": filter_str})
    ...
    async def rename_local_variable(self, address: str, old_name: str, new_name: str) -> bool:
        res = await self._call("ghidra_rename_local_variable", {"address": address, "old_name": old_name, "new_name": new_name})
        return res.get("success", False)

    async def set_local_variable_type(self, address: str, variable_name: str, new_type: str) -> bool:
        res = await self._call("ghidra_set_local_variable_type", {"address": address, "variable_name": variable_name, "new_type": new_type})
        return res.get("success", False)

    async def patch_bytes(self, address: str, hex_bytes: str) -> bool:
        res = await self._call("ghidra_patch_bytes", {"address": address, "hex_bytes": hex_bytes})
        return res.get("success", False)
